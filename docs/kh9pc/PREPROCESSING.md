...
If either side fails to cross the gradient threshold, both are retried together with the search window shifted right by half its width, up to 5 attempts before giving up.
...
- **Left/right edges** ([2.3](#23-detect-left-and-right-edges)): if either side fails to cross the gradient threshold, both are retried together with a shifted search window, up to 5 attempts. If detection still fails after all attempts, fitting aborts immediately with an exception — every other detector depends on these bounds.
